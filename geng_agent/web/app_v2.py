...
from .artifacts import LocalArtifactStore, UnsafeArtifactPath, manifest_hash, preview_artifact
...
def _dispatch_review(job_id: str) -> None:
    if settings.celery_eager:
        threading.Thread(target=run_review.run, args=(job_id,), name=f"geng-eager-{job_id[:8]}", daemon=True).start()
        return
    try:
...
async def create_case(
    ...
    job = JobRecord(
        ...
        case_id=case_id,
        status="queued",
        options={"run_repro": True, "result_review": True, "template_fallback": True, "per_task_layout": True},
    )
...
def create_export(case_id: str, phase: str | None = None, session: Session = Depends(get_session)) -> dict[str, Any]:
    ...
    session.commit()
    if settings.celery_eager:
        threading.Thread(target=build_export.run, args=(export.id,), daemon=True).start()
    else:
