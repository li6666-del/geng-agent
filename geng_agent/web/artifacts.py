...
STEP_FILES: dict[str, tuple[str, ...]] = {
    "paper": ("paper_chunks.json", "paper/"),
    "engineering_facts": ("engineering_facts.json",),
    "repro_tasks": ("repro_tasks.json",),
    ...
    "runtime": ("runtime_result.json", "runtime_result_pre_fallback.json", "repro_project/repair_logs/"),
    "result_review": ("result_review.json", "result_review.md", "result_review.docx", "result_review_error.json"),
    "review": ("risk_report.json", "review.md", "review.docx", "generated_files.json", "run_cost.json"),
}
...
class LocalArtifactStore:
    ...
    def iter_files(self) -> Iterable[Path]:
        ...
        for path in self.case_dir.rglob("*"):
            ...
            if any(part.startswith(".") for part in rel.parts) or rel.parts[0] == "exports":
                continue
            yield path
        ...
        for prefix in prefixes:
            if normalized == prefix.rstrip("/") or normalized.startswith(prefix):
                ...
                if best is None or candidate[0] > best[0]:
                    best = candidate
    return best[1] if best else "project_build"
