...
from geng_agent.config import build_code_review_client, build_llm_client, build_secondary_extraction_client
...
from .pipeline_runner import InstrumentedReviewPipeline
...
def run_review(self, job_id: str) -> None:
    ...
    try:
        pipeline = InstrumentedReviewPipeline(
            client=build_llm_client(),
            code_review_client=build_code_review_client(),
            extraction_client_2=build_secondary_extraction_client(),
            progress=reporter,
        )
        pipeline.run(
            ...
            result_review=bool(options.get("result_review", True)),
            resume=True,
            template_fallback=bool(options.get("template_fallback", True)),
            per_task_layout=bool(options.get("per_task_layout", True)),
        )
        with SessionLocal() as session:
            job = session.get(JobRecord, job_id)
