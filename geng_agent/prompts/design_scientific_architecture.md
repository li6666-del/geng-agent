...
Describe the science needed to implement the finalized tasks. The host records this plan and does not complete descriptive fields, infer scientific equivalence or rewrite defaults. Keep private tasks local when no shared implementation is scientifically needed; the next consumer raises a specific blocker only when it cannot execute the handoff.
...
4. Declare each implementation `module` as a safe relative Python path under `src/`. Never use an absolute path or `..`. Implementation files belong to each task project. `inputs`, `outputs`, `parameters` and `depends_on` refer to the IDs you declare.
...
6. Respect the task boundaries chosen by balancing scientific dependencies, Writer workload, independent review and repair, and parallel execution benefits. Component reuse or compatible implementations must not silently merge tasks or add experiments. Each task owns a complete implementation. Describe state that must be developed together inside its task; for fixed cross-task inputs use the task depends_on declaration. Do not create shared-code ownership, freezing rules or strong/weak task groups.
...
8. Optional `acceptance_bindings` connect a task's criterion ID to a measurable output; they do not redefine acceptance. Qualitative claims need no invented numerical proxy. Reporter independently checks implementation, comparability and task outcome from the paper and execution evidence. The supervisor handles only an actual handoff or recovery blocker.
...
This is a mechanical one-to-one index of the final tasks chosen by the planner. It does not merge tasks or impose scientific ownership. Architecture describes components and experiment variants within those tasks.
