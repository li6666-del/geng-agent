...
10. 数值稳健性（核心：防止“能跑通但算错”或中途崩溃，这是最常见的兜底来源）：
    ...
    - 物理标定 / SNR 约定（避免“某指标整张图恒为 0”这类退化，2603 LEO 卫星论文真实踩过）：通信论文里 SNR、Eb/N0、发射功率/SNR 轴等通常是**归一化/相对**量——把它当“工作信噪比”直接用（令噪声方差 σ²=1，或把信道归一化成单位平均增益，再按论文的 SNR 设信号功率），**不要**用裸物理量（真实 Friis 路损 `c/(4πf d)` × 真实热噪声 `kTB`/`N0·B`）去算绝对链路预算：后者常给出 −数十 dB 的退化工作点，让 SINR≈0、`log2(1+SINR)≈0`、整张图恒为 0。除非论文明确给了绝对功率/距离/噪声谱密度并要求绝对链路预算，否则一律按论文的归一化/相对 SNR 约定来，功率/SNR 横轴照论文范围扫。
