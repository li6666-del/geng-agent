...
class ReporterObservationTests(unittest.TestCase):
    def test_post_execution_image_cannot_support_claim_without_observed_data(self) -> None:
        with TemporaryDirectory() as temp:
            ...
            self.assertTrue(checked["_engineering_issues"])
    ...
    def test_real_source_proof_preserves_method_failure_without_outputs(self) -> None:
        with TemporaryDirectory() as temp:
            ...
            raw = {"schema_version": "3.0", "task_id": "experiment_a", "run_valid": True, "host_action": "complete", "decision_reason": "Independent observation of the supplied method and measurements.", "outcome": "not_reproduced",
                "core_conclusions": [{
            ...
            raw["host_action"] = "rerun_writer"
            raw["core_conclusions"].insert(0, {"claim_id": "ordering", "status": "unsupported", "local_observation": "The method is a constant.", "evidence_files": [source.relative_to(workspace).as_posix()]})
    ...
    def test_top_level_real_output_can_support_a_claim_without_duplicate_paths(self) -> None:
        with TemporaryDirectory() as temp:
            ...
            checked, warnings = normalize_reporter_observation_evidence({"schema_version": "3.0", "task_id": "experiment_a", "run_valid": True, "host_action": "complete", "decision_reason": "Independent observation of the supplied method and measurements.", "outcome": "reproduced",
    ...
    def test_unsupported_missing_evidence_is_retained_without_authorizing_rerun(self) -> None:
        with TemporaryDirectory() as temp:
            workspace = Path(temp)
            checked, warnings = normalize_reporter_observation_evidence({"schema_version": "3.0", "task_id": "experiment_a", "run_valid": True, "host_action": "complete", "decision_reason": "Independent observation of the supplied method and measurements.", "outcome": "not_reproduced",
